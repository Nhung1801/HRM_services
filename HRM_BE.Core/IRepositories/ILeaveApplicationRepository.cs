...
namespace HRM_BE.Core.IRepositories
{
    ...
    {
        Task<PagingResult<LeaveApplicationDto>> GetPaging(int? organizationId, int? employeeId, DateTime? startDate, DateTime? endDate, double? numberOfDays,int? typeOfLeaveId,string? reasonForLeave,string? note, LeaveApplicationStatus? status, string? sortBy, string? orderBy, int pageIndex = 1, int pageSize = 10);
