...
namespace HRM_BE.Api.Controllers.Address
{
    [Route("api/address")]
    [ApiController]
    public class AddressController : ControllerBase
    {
        ...
        [HttpGet("get-country")]
        public async Task<ApiResult<List<Country>>> GetCountry()
        {
            List<Country> cities = await _unitOfWork.Address.GetCountries();
            return new ApiResult<List<Country>>()
            {
                Status = true,
                Message = "Thành công",
                Data = cities
